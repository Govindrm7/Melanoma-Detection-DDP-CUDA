...
def train_model(rank, world_size, data_dir, epochs, batch_size, timings, gpu_count):
    ...
    # Prepare log file
    log_file = f'training_metrics_gpu_{gpu_count}.json'  # Log file for metrics
    ...
    for epoch in range(epochs):
        ...
        if rank == 0:
            print(f"Rank {rank}, GPU Count {gpu_count}, Epoch {epoch+1}/{epochs}, "
    ...
    if rank == 0:
        timings[gpu_count] = total_training_time
...
def main():
    ...
    timings = {}  # Dictionary to store training times for each GPU count
    max_gpus = 4  # Run experiments on up to 4 GPUs

    for gpu_count in range(1, max_gpus + 1):
        print(f"Starting training on {gpu_count} GPUs...")
        mp.spawn(
            train_model,
            args=(gpu_count, data_dir, epochs, batch_size, timings, gpu_count),
            nprocs=gpu_count,
            join=True
        )
    
    # Calculate Speedup and Efficiency
    baseline_time = timings[1]  # Training time on 1 GPU as the baseline
    speedups = {g: baseline_time / t for g, t in timings.items()}
    efficiencies = {g: speedups[g] / g for g in speedups}
    ...
    results_file = 'ddp_results.json'
    with open(results_file, 'w') as f:
        json.dump({'timings': timings, 'speedups': speedups, 'efficiencies': efficiencies}, f)
